...
- 当前正式版：v1.7.5+19，基于 GitHub 发布标签 `v1.7.4`；发布标签为 `v1.7.5`。
- v1.7.5 处理 Android 回到前台后偶发无响应：后台暂停 UDP 发现和自动重连，恢复前台才有序启动；广播暂时失败不再反复销毁并重建 UDP socket。公网协议仍为 3，VPS 不需要重新部署；Android `1.7.5+18` 测试版已由用户实机确认正常。
...
先核对 `docs/开发交接说明-v1.7.5.md` 的 Android 后台恢复修复、发布校验值与 VPS 现状；不得改动原个人网站 Caddy 站点。当前优先让用户完成 Android 后台停留后返回前台的实机回归，任何密码、令牌或私钥都不得提交到 Git。
