...
- v1.6.1 稳定版保持纯局域网；v1.7.0 开发分支允许接入用户自有 VPS，但不引入 S3、公共账号系统或剪贴板同步。
...
- 公网协议：`protocolVersion = 1`，与局域网协议独立
- 测试数量：应用 22 项、远程协议 3 项、中转服务 4 项
- UDP 发现：53317
- TCP/HTTP/WebSocket：53318
- 所有参与传输的设备必须使用相同协议版本。
- 公网传输必须明确显示实际路线，局域网优先；公网图片发送必须允许停止并清理未完成文件。
